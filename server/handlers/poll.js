...
exports.createPoll = async (req, res, next) => {
  ...
  try {
    ...
    return res
      .status(201)
      .json({ ...newPoll._doc, user: user._id, test: "retststss" });
  } catch (error) {
    next(error);
  }
};
...
exports.deletePoll = async (req, res, next) => {
  ...
  try {
    ...
    if (poll.user.toString() !== idUser)
      throw new Error("cant delete with wrong user");
    await poll.remove();
    await user.save();
...
exports.getPoll = async (req, res, next) => {
  const { id } = req.params;
  try {
    const result = await db.Poll.findById(id).populate("user", [
      "id",
      "username"
    ]);
